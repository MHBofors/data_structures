...
darray_t *array_alloc(size_t element_size) {
	...
	if ((array = malloc(sizeof(darray_t))) &&
	    (data = malloc(ARRAY_INIT_SIZE * element_size))) {
		*array = (darray_t){.element_size = element_size,
		                    .element_count = 0,
...
bool array_append(darray_t *array, void *append_source, size_t append_count) {
	...
	memcpy((char *)array->data + array->element_size * array->element_count,
	       append_source, array->element_size * append_count);
	array->element_count += append_count;

	return true;
}

bool array_insert(darray_t *array, void *insert_source, size_t insert_index,
                  size_t insert_count) {
	...
	memmove((char *)array->data +
	            array->element_size * (insert_index + insert_count),
	        (char *)array->data + array->element_size * insert_index,
	        array->element_size * (array->element_count - insert_index));
	memcpy((char *)array->data + array->element_size * insert_index,
	       insert_source, array->element_size * insert_count);
...
bool array_delete(darray_t *array, size_t delete_index, size_t delete_count) {
	if (array->element_count < delete_index + delete_count ||
	    delete_count == 0) {
		return false;
	} else {
		memmove((char *)array->data + array->element_size * delete_index,
		        (char *)array->data +
		            array->element_size * (delete_index + delete_count),
		        array->element_size *
		            (array->element_count - (delete_index + delete_count)));
...
bool array_replace(darray_t *array, void *replace_source, size_t replace_index,
	...
	if (array->element_count < replace_index + replace_count ||
	    replace_count == 0) {
		return false;
	} else {
		memcpy((char *)array->data + array->element_size * replace_index,
		       replace_source, array->element_size * replace_count);
		return true;
	}
}
...
size_t array_size(darray_t *array) { return array->element_count; }
