...
size_t graph_vertex_out_edge_count(graph_t *graph, size_t vertex_index) {
    ...
    return graph->row_indices[vertex_index + 1] - graph->row_indices[vertex_index];
}



bool graph_is_adjacent(graph_t *graph, size_t from_vertex_index, size_t to_vertex_index) {
    if(from_vertex_index >= graph->vertex_count || to_vertex_index >= graph->vertex_count) {
        return false;
    }
...
bool graph_add_vertex(graph_t *graph, void *vertex_data) {
    ...
    if(graph->vertex_capacity + 1 <= graph->vertex_count) {
        ...
        if((new_vertex_array = realloc(graph->vertex_array, new_capacity * graph->vertex_size)) && (new_row_indices = realloc(graph->row_indices, new_capacity * sizeof(size_t)))) {
    ...
    if(graph->vertex_size != 0) {
        memcpy((char *)graph->vertex_array + graph->vertex_size*graph->vertex_count, vertex_data, graph->vertex_size);
    }
...
bool graph_insert_edge(graph_t *graph, size_t from_vertex_index, size_t to_vertex_index, void *edge_data) {
    ...
    if(graph->edge_capacity <= graph->edge_count) {
        ...
        if((new_edge_array = realloc(graph->edge_array, new_capacity * graph->edge_size)) && (new_column_indices = realloc(graph->column_indices, new_capacity * sizeof(size_t)))) {
    ...
    memmove(graph->column_indices + sizeof(size_t)*(graph->row_indices[from_vertex_index + 1] + 1),
            graph->column_indices + sizeof(size_t)*graph->row_indices[from_vertex_index + 1],
            sizeof(size_t)*(graph->edge_count - graph->row_indices[from_vertex_index]));
        
    if(edge_data != NULL) {
        memmove((char *)graph->edge_array + graph->edge_size*(graph->row_indices[from_vertex_index + 1] + 1),
                (char *)graph->edge_array + graph->edge_size*graph->row_indices[from_vertex_index + 1],
                graph->edge_size*(graph->edge_count - graph->row_indices[from_vertex_index]));
        
        memcpy((char *)graph->edge_array + graph->edge_size*graph->row_indices[from_vertex_index + 1], edge_data, graph->edge_size);
    }
...
bool graph_set_edge(graph_t *graph, size_t from_vertex_index, size_t to_vertex_index, void *edge_data) {
    
}



void *graph_get_vertex(graph_t *graph, size_t vertex_index, void *out_vertex_data) {
    if(vertex_index >= graph->vertex_count) {
        return NULL;
    }

    void *vertex_ptr = (char *)graph->vertex_array + graph->vertex_size*vertex_index;
...
void *graph_get_edge(graph_t *graph, size_t from_vertex_index, size_t to_vertex_index, void *out_edge_data) {
    size_t row_start = graph->row_indices[from_vertex_index];
    size_t row_end = graph->row_indices[from_vertex_index + 1];
